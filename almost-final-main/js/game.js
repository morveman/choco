...
function doGame(e){
  ...
  if (playIA.checked == true){
    ...
    if (NombrePosition(newGrid) == 0){
      ...
      setTimeout(() => {  showEndSreen("Player " + String(playing_player + 1) + " won the game",1); }, 200);
    }
  }
};
...
function IA_checked(newGrid){
  if (NombrePosition(newGrid) != 0){
    ...
    setTimeout(() => {
      newGrid = IA_play(50);
        if (NombrePosition(newGrid) == 0){
          ...
          setTimeout(() => {showEndSreen("You won ! Well played",1);}, 1000);
    ...
    //showEndSreen
    setTimeout(() => {showEndSreen("The IA won ! Sorry... ",1);}, 1000);
  }
};
